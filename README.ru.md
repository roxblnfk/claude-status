...
`opus`, `sonnet` и `haiku` — это алиасы, и Claude Code сам решает, какой релиз
стоит за каждым: выходит новый — сессии переезжают на него. В **«Настройки →
Модели»** алиас направляется на конкретный релиз, так что `/model opus`
продолжает работать и приводит туда, где вам удобно. Там же задаётся модель, с
которой начинается сессия, и та, на которой работают сабагенты. Какие модели
доступны вашей подписке, знает только Claude Code, поэтому там ничего не
проверяется: имя, которое он не принимает, проявится сессией, отказавшейся
запускаться.
